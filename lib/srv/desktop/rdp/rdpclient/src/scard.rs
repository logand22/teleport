...
impl EstablishContext_Return {
    ...
    fn encode(&self) -> RdpResult<Vec<u8>> {
        let mut w = vec![];
        w.write_i32::<LittleEndian>(self.return_code.to_i32().unwrap())?;
        w.append(&mut self.context.encode_ptr(&mut 0)?);
        w.append(&mut self.context.encode_value()?);
        Ok(encode_response(encode_struct(w)?)?)
    }
}
...
impl Context {
    ...
    fn decode_ptr(payload: &mut Payload, index: &mut u32) -> RdpResult<Self> {
        let (length, _) = decode_ptr(payload, index)?;
        Ok(Self {
            length: length,
            value: 0,
        })
...
fn decode_ptr(payload: &mut Payload, index: &mut u32) -> RdpResult<(u32, u32)> {
    let length = payload.read_u32::<LittleEndian>()?;
    let ptr = payload.read_u32::<LittleEndian>()?;
    let expect_ptr = 0x00020000 + *index * 4;
    *index += 1;
    if ptr != expect_ptr {
        Err(invalid_data_error("invalid NDR pointer value"))
    } else {
        Ok((length, ptr))
    }
}
...
impl ListReaders_Call {
    fn decode(payload: &mut Payload) -> RdpResult<Self> {
        ...
        let (groups_ptr_length, _groups_ptr) = decode_ptr(payload, &mut index)?;
        ...
        let (groups_length, groups) = decode_multistring_utf16(payload)?;
...
impl ListReaders_Return {
    ...
    fn encode(&self) -> RdpResult<Vec<u8>> {
        let mut w = vec![];
        w.write_i32::<LittleEndian>(self.return_code.to_i32().unwrap())?;
        let mut readers = encode_multistring_utf16(&self.readers)?;
        let mut ptr = encode_ptr(readers.length() as u32, &mut 0)?;
        w.append(&mut ptr);

        w.write_u32::<LittleEndian>(readers.length() as u32)?;
        w.append(&mut readers);

        Ok(encode_response(encode_struct(w)?)?)
    }
}

fn decode_multistring_utf16(payload: &mut Payload) -> RdpResult<(u32, Vec<String>)> {
...
fn encode_multistring_utf16(items: &Vec<String>) -> RdpResult<Vec<u8>> {
    let mut buf = vec![];
    for s in items.iter() {
...
fn print_payload(payload: &mut Payload) {
